...
where
    ...
    S: OutputPin,
{
    type Error = SpiTransportError<B::Error>;

    async fn reset(&mut self) -> Result<(), Self::Error> {
        // Pull SDB low to enter hardware shutdown, then release.
        self.sdb.set_low().map_err(|_ignored| SpiTransportError::Pin)?;
        embassy_time::Timer::after_micros(250).await;
        self.sdb.set_high().map_err(|_ignored| SpiTransportError::Pin)?;
    ...
    async fn write_page(&mut self, driver_index: usize, page: u8, reg: u8, data: &[u8]) -> Result<(), Self::Error> {
        if data.len() > PWM_REGISTER_COUNT {
            return Err(SpiTransportError::PayloadTooLarge);
        }

        let mut buf = [0_u8; PWM_REGISTER_COUNT.saturating_add(2)];
        let Some(cmd) = buf.get_mut(0) else { return Err(SpiTransportError::InvalidIndex) };
        *cmd = WRITE_CMD | PATTERN_CMD | (page & 0x0F);
        let Some(reg_slot) = buf.get_mut(1) else { return Err(SpiTransportError::InvalidIndex) };
        *reg_slot = reg;
        ...
        let Some(cs) = self.cs.get_mut(driver_index) else {
            return Err(SpiTransportError::InvalidIndex);
        };
        cs.set_low().map_err(|_ignored| SpiTransportError::Pin)?;

        let result = self.bus.write(buf.get(..payload_end).unwrap_or(&[])).await.map_err(SpiTransportError::Spi);
    ...
    async fn read_reg(&mut self, driver_index: usize, page: u8, reg: u8) -> Result<u8, Self::Error> {
        ...
        let result = self.bus.transfer(&mut rx, &tx).await.map_err(SpiTransportError::Spi);

        let _ = self.cs.get_mut(driver_index).map(OutputPin::set_high);
        result?;

        rx.get(2).copied().ok_or(SpiTransportError::InvalidIndex)
    }
}
